...
int main(void) {
  ...
  */
  ...
  // Declare "mpz_t" variables to store the values ​​of M, e, n and C
  char input[10000], output[1000];
  printf("\nDo you want to encrypt and decrypt a message? (Y/N)");
  scanf("%s", &input);
  ...
  if(input[0] != 'N' && input[0] != 'n'){
    ...
    printf("\nInsert the message (only decimal values): ");
    scanf("%s", input);
    mpz_init_set_str(message, input, 10);

    // C = M^e mod n
    start = clock();
    mpz_powm(encrypted, message, e, n);
    end = clock();
    calculateTime(end, start, "\nTime encrypting the message:");


    //Decrypt mpz_powm(M, C, d, n)
    start = clock();
